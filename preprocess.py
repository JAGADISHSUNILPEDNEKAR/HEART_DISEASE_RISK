...
class FeatureEngineer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        for col in ['fbs', 'exang']:
            if col in X.columns:
                 X[col] = X[col].replace({'TRUE': 1, 'FALSE': 0, 'True': 1, 'False': 0, True: 1, False: 0}).astype(float)
...
class HeartDiseasePreprocessor:
    ...
    def _build_pipeline(self):
        ...
        self.new_numeric_features = ['chol_age_ratio', 'bp_above_140', 'oldpeak_high', 'stress_reserve', 'ca_binary']
    ...
    def load(self, filepath=PREPROCESSOR_FILE):
